...
#    df.loc[i,'Date_Use'] = datetime.strptime(df.iloc[i,1], ' "%b %d/%Y"')
...
#df_use = df_use.set_index('Timeindex')
...
# check if stationary in the original time series
...
#interval = 1
#diff_series = difference(df_use['Variable'], interval=interval)
#diff_series.index = df_use.loc[interval:,'Timeindex']
#
## check if stationary
...
#plt.figure()
#plt.plot(diff_series.index[0:24*10],diff_series[0:24*10])
#plt.show()
...
#plt.figure()
#plt.subplot(211)
#plot_acf(diff_series[0:24*100],lags=np.arange(100), ax=plt.gca())
#plt.subplot(212)
#plot_pacf(diff_series[0:24*100],lags=np.arange(100), ax=plt.gca())
#plt.show()
...
#        try:
#            print(param)
#            mod = sm.tsa.statespace.SARIMAX(train['Variable'], order=param, seasonal_order=param_seasonal,enforce_stationarity=False,
#                                enforce_invertibility=False)
#            results = mod.fit()
#            print('ARIMA{}x{}12 - AIC:{}'.format(param, param_seasonal, results.aic))
#            mod_list.append([param, param_seasonal, results.aic])
#        except:
#            continue

## Doing fourier transform of series
X = scipy.fft(train['Variable'])
Y = scipy.zeros(len(X))
